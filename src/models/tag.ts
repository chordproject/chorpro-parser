...
export default class Tag {
  ...
  static parse(tag: string) {
    return (
      this.parseWithRegex(tag, META_TAG_REGEX) ||
      this.parseWithRegex(tag, TAG_REGEX)
    );
  }

  static parseWithRegex(tag: string, regex: RegExp) {
   ...
   * @returns {boolean}
   */
  public isMetaTag(): boolean {
    return (
      CUSTOM_META_TAG_NAME_REGEX.test(this.name) ||
      META_TAGS.indexOf(this.name) !== -1
    );
  }
