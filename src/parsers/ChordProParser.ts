...
import { CommentLine } from '../models/CommentLine';
import { CustomLine } from '../models/CustomLine';
import { EmptyLine } from '../models/EmptyLine';
import { Key } from '../models/Key';
import { Line } from '../models/Line';
import { LyricsLine } from '../models/LyricsLine';
import { Lyrics } from '../models/sections/Lyrics';
import { LyricsBase, LyricsType } from '../models/sections/LyricsBase';
import { Section, SectionType } from '../models/sections/Section';
import { SimpleLyrics } from '../models/sections/SimpleLyrics';
import { Tabs } from '../models/sections/Tabs';
import { TabLine } from '../models/TabLine';
...
export class ChordProParser {
	 ...
	 */
	private get warnings(): ParserWarning[] {
		...
		if (this._currentSection instanceof LyricsBase) {
			...
			lineParts.forEach((part) => {
				...
				if (!result[0]) {
					...
					if (lyrics) {
						this.addWarning(
							`Cannot parse the chord '${
								result[1].text
							}' before the lyrics '${result[1].lyrics.trim()}'`
						);
					} else {
	...
	private parseTag(line: string) {
		...
		switch (tag.type) {
			...
			case TagType.EndOfBlock:
				this.parseEndOfBlockTag(tag.longName, tag.value);
				break;
			default:
	...
	private parseStartOfBlockTag(longName: string, value: string | null) {
		...
		this.completeCurrentSection();
		this._currentSectionTagName = longName;
		const blockType = TagConstants.START_BLOCK_TAGS.find((f) => f === longName);

		// custom section
		if (!blockType) {
			this._currentSection = new Lyrics(LyricsType.Custom, longName, value);
			return;
		}
		...
		switch (blockType) {
			case TagConstants.START_OF_BRIDGE:
				this._currentSection = new Lyrics(LyricsType.Bridge, longName, value);
				return;
			case TagConstants.START_OF_CHORUS:
				this._currentSection = new Lyrics(LyricsType.Chorus, longName, value);
				return;
			case TagConstants.START_OF_VERSE:
				this._currentSection = new Lyrics(LyricsType.Verse, longName, value);
				return;
	 ...
	 * @param value Tag value
	 */
	private parseEndOfBlockTag(name: string, value: string | null) {
