...
test.each`
  ...
  ${"G"} | ${MusicLetter.G}
`(
  "parse the simple key $key and return the correct key",
  ({ key, expected }) => {
...
test.each`
  ...
  ${"Gb"} | ${MusicLetter.G}
`(
  "parse the bemol key $key and return the correct key",
  ({ key, expected }) => {
    const expectedKey = new Key(
      new MusicNote(expected, MusicAccidental.b),
      KeyMode.Major
    );
    const resultKey = Key.parse(key);
    expect(resultKey).toMatchObject(expectedKey);
...
test.each`
  ...
  ${" G# "} | ${MusicLetter.G}
`(
  "parse the sharp key $key and return the correct key",
  ({ key, expected }) => {
    const expectedKey = new Key(
      new MusicNote(expected, MusicAccidental["#"]),
      KeyMode.Major
    );
    const resultKey = Key.parse(key);
    expect(resultKey).toMatchObject(expectedKey);
...
describe("return the key as text", () => {
  ...
  it("major key", () => {
    const key = new Key(
      new MusicNote(MusicLetter.A, MusicAccidental["#"]),
      KeyMode.Major
    );
    const keyString = key.toString();
